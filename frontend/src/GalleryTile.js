...
export default function GalleryTile({ media, mediaIdx, likes, selectMedia, showLike }) {
  ...
  return (
    ...
    >
      ...
      {media && <ImageListItemBar sx={{ background: "none" }} title={media.caption} actionIcon={
        showLike &&
        <IconButton
          ...
          onClick={function (event) {
            ...
            fetch(`/likes?photo=${media.id}`).then(() => {
              mutate("/likes")
            }).catch(() => {
        ...
        >
          <Badge badgeContent={likes[media.id]} >
            {localStorage.getItem(media.id) ? <FavoriteIcon /> : <FavoriteBorderIcon />}
          </Badge>
        </IconButton>
      } />}
    </ImageListItem>
